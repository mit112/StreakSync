---
name: StreakSync Social System Redesign – Implementation Plan
overview: ""
todos:
  - id: b8595e40-a709-4554-8da6-b4240d31d4eb
    content: Soft-deprecate friend-code APIs and UI, add instrumentation, and communicate upcoming changes in-app.
    status: pending
  - id: 19ead8cb-9f95-4452-b94d-7f5e1be61cef
    content: Define and implement a unified CloudKitSocialService API with local caching that replaces HybridSocialService and MockSocialService over time.
    status: pending
  - id: a5d734d8-b3ef-4b05-8be5-6adb999bfbab
    content: Add permissions flow and CK-based contact discovery, plus caching and manual username fallback.
    status: pending
  - id: e2799ec3-ac3c-4995-9b1a-fe2a5019c7dd
    content: Add Circle data model, CloudKit schema, and UI for creating/joining/leaving circles and filtering leaderboards.
    status: pending
  - id: 21e2c8c0-a310-4b68-a6e3-d8e8f9d0b55e
    content: Introduce per-game social privacy settings, settings UI, and enforcement in score publishing and aggregation.
    status: pending
  - id: 3db8bdb2-8694-4e36-b773-c84531cb7b80
    content: Implement reactions, activity feed, and related notifications on top of the new social model.
    status: pending
  - id: e328090f-85d2-41c2-923d-2189f135d823
    content: Implement caching, incremental sync, offline queuing, and background refresh for social data.
    status: pending
  - id: ce7027ac-5b9e-429e-b133-9c263cc57ef2
    content: Refactor Friends tab into leaderboard/friends list/circles views with the new IA and visual hierarchy.
    status: pending
  - id: cd65d216-d336-4c27-95e6-a0a141ac436e
    content: Add automated tests, run phased TestFlight and staged production rollout with feature flags and rollback support.
    status: pending
---

# StreakSync Social System Redesign – Implementation Plan
...
## Step 1 – Deprecate Friend Codes (Non-breaking)
...
- Replace friend-code CTAs with a short, dismissible notice: “Finding friends will soon be automatic via your contacts. Existing connections stay intact.”
...
- Implement functions to map existing `UserProfile` → `User`, existing “friend codes” list → `[Friendship]`, and the current single shared group → initial `Circle`.
...
## Step 4 – Contact Discovery & Permissions

1. **Permissions UX design**

- Design a dedicated “Find Friends Who Play” screen (new view in `Features/Friends/Views/`) matching the flow in your plan (Not Now / Find Friends, explainer text, etc.).
...
- Expose this in a secondary UI path (e.g., an “Add by username” field in Friends screen), clearly labelled as advanced.

## Step 5 – Circles (Multiple Groups)
...
- During migration, convert the existing `LeaderboardGroupStore.selectedGroupId` into a first `Circle` named “Friends” and associate existing scores with that circle.

## Step 6 – Circle-based Leaderboards
...
- Support “All Friends” (circle == nil) by aggregating across all circles/friendships.

2. **FriendsViewModel refactor**

- Replace current group-based global leaderboard calls with circle-aware calls.
- Introduce state for currently selected circle (and default to “All Friends”).
- Ensure rank-delta logic still works with per-circle leaderboards.

3. **UI behavior**

- In `FriendsView`, add a circle selector to the header.
- Ensure empty states and CTAs are circle-aware (e.g., “No scores in this circle yet. Invite friends.”).

## Step 7 – Privacy Controls & Sharing Rules
...
- In the Settings tab (likely under `Features/Settings/`), create a “Social & Privacy” screen with per-game sharing controls and global toggles (e.g., “Hide incomplete games”).

3. **Enforcement in publish path**

- Update the score publishing pipeline in `AppState` → `CloudKitSocialService.publishScore` to filter out scores that should not be published according to privacy settings.
...
## Step 8 – Enhanced Social Features (Reactions, Activity Feed)
...
- Add quick reaction buttons on leaderboard rows (e.g., “👏”, “🔥”).
...
## Step 9 – Performance, Caching, and Offline Support
...
- Replace friend-code flows with the new “From Contacts” and “Your Friends” lists, plus invitations and circle management screens.
...
## Step 11 – Testing, Beta, and Rollout
...
- Use TestFlight phases (internal → power users → broader beta) and collect feedback via in-app prompts/analytics.
...
## Step 12 – Post-Launch Monitoring & Iteration
