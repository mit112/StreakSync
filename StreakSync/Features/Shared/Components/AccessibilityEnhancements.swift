...
struct AccessibilityAnnouncer {
    ...
    static func announceAchievementUnlocked(_ achievement: TieredAchievement, tier: AchievementTier) {
        let message = "Achievement unlocked: \(achievement.displayName) at \(tier.displayName) level"
        announce(message)
    }
