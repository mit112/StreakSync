...
final class AchievementCheckerTests: XCTestCase {
    ...
    /// Builds an AchievementSnapshot and calls checkAllAchievements with the new API.
    private func check(
        results: [GameResult],
        streaks: [GameStreak] = [],
        games: [Game] = [],
        ...
        let snapshot = AchievementSnapshot.build(from: results, games: games)
    ...
    func testEarlyBirdCountsMorningPlays() {
        ...
        _ = check(results: all, achievements: &achievements)
    ...
    func testEarlyBirdExcludesBoundary() {
        ...
        _ = check(results: [result], achievements: &achievements)
